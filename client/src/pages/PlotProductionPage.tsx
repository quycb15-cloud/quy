...
export default function PlotProductionPage() {
  ...
  return <div className="page-enter min-w-0 max-w-full overflow-x-hidden">
    <PageHeader eyebrow="Báo cáo sản lượng" title="Sản lượng theo lô" description="Nhập Mủ đông/tạp và Quy khô theo từng Lô có sẵn trong hệ thống; tra cứu theo năm, tháng và Đội." action={<Button variant="outline" className="min-h-11 w-full bg-white sm:w-auto" onClick={exportReport}><Download className="mr-2 h-4 w-4" />Xuất Excel</Button>} />
      ...
      <div className="space-y-5">
        <Panel title="Nhập sản lượng theo lô" description="Chọn đúng tên Lô và lập số liệu theo Tháng/Năm; mỗi Lô chỉ có một bản ghi cho một tháng.">
          <form onSubmit={submit} className="grid gap-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2"><Field label="Tháng"><select value={form.month} onChange={event => setForm(current => ({ ...current, month: event.target.value }))} className="min-h-11 w-full rounded-md border border-input bg-background px-3 text-sm">{Array.from({ length: 12 }, (_, index) => index + 1).map(item => <option key={item} value={item}>Tháng {item}</option>)}</select></Field><Field label="Năm"><select value={form.year} onChange={event => setForm(current => ({ ...current, year: event.target.value }))} className="min-h-11 w-full rounded-md border border-input bg-background px-3 text-sm">{years.map(item => <option key={item} value={item}>Năm {item}</option>)}</select></Field></div>
            ...
            <Button disabled={create.isPending || manualPeriodLocked} className="min-h-11 w-full bg-emerald-700 hover:bg-emerald-800">{create.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}{manualPeriodLocked ? "Kỳ đã chốt" : create.isPending ? "Đang lưu…" : "Lưu sản lượng"}</Button>
          </form>
        </Panel>
        <Panel title="Nhập nhanh nhiều Lô" description="Nhập nhiều Lô cùng một Đội và cùng Tháng/Năm để lưu một lần.">
          <form onSubmit={submitQuickEntries} className="grid gap-3">
            <div className="grid gap-3 sm:grid-cols-3"><Field label="Đội"><select value={quickUnit} onChange={event => { setQuickUnit(event.target.value); setQuickEntries([]); }} className="min-h-11 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="">Chọn Đội</option>{units.map(item => <option key={item} value={item}>{item}</option>)}</select></Field><Field label="Tháng"><select value={quickMonth} onChange={event => setQuickMonth(event.target.value)} className="min-h-11 w-full rounded-md border border-input bg-background px-3 text-sm">{Array.from({ length: 12 }, (_, index) => index + 1).map(item => <option key={item} value={item}>Tháng {item}</option>)}</select></Field><Field label="Năm"><select value={quickYear} onChange={event => setQuickYear(event.target.value)} className="min-h-11 w-full rounded-md border border-input bg-background px-3 text-sm">{years.map(item => <option key={item} value={item}>Năm {item}</option>)}</select></Field></div>
            ...
            <div className="space-y-3">{quickEntries.map((entry, index) => <div key={index} className="rounded-xl border border-slate-100 bg-slate-50 p-3"><div className="grid gap-3 sm:grid-cols-[minmax(0,1.8fr)_minmax(0,1fr)_minmax(0,1fr)_auto]"><Field label="Lô"><select value={entry.plotId} onChange={event => updateQuickEntry(index, { plotId: event.target.value })} disabled={quickPeriodLocked} className="min-h-11 w-full rounded-md border border-input bg-white px-3 text-sm disabled:opacity-50"><option value="">Chọn Lô</option>{quickPlots.map(plot => <option key={plot.id} value={plot.id} disabled={quickEntries.some((item, itemIndex) => itemIndex !== index && item.plotId === String(plot.id))}>{plot.name} ({plot.plantedYear ?? "—"})</option>)}</select></Field><Field label="Mủ đông/tạp"><Input type="number" min="0" step="0.01" value={entry.frozenContaminatedLatex} onChange={event => updateQuickEntry(index, { frozenContaminatedLatex: event.target.value })} disabled={quickPeriodLocked} required /></Field><Field label="Quy khô"><Input type="number" min="0" step="0.01" value={entry.dryRubber} onChange={event => updateQuickEntry(index, { dryRubber: event.target.value })} disabled={quickPeriodLocked} required /></Field><Button type="button" variant="ghost" size="icon" className="mt-7 min-h-11 min-w-11 text-slate-500 hover:text-red-600" onClick={() => setQuickEntries(current => current.filter((_, entryIndex) => entryIndex !== index))} aria-label="Xóa Lô" disabled={quickPeriodLocked}><Trash2 className="h-4 w-4" /></Button></div><Input className="mt-3" value={entry.note} onChange={event => updateQuickEntry(index, { note: event.target.value })} placeholder="Ghi chú cho Lô (nếu có)" disabled={quickPeriodLocked} /></div>)}</div>
            <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" className="bg-white" onClick={addQuickEntry} disabled={!quickUnit || quickPeriodLocked || quickEntries.length >= 200}><Plus className="mr-2 h-4 w-4" />Thêm Lô</Button><Button disabled={!quickEntries.length || quickCreate.isPending || quickPeriodLocked} className="min-h-11 w-full bg-emerald-700 hover:bg-emerald-800 sm:w-auto">{quickCreate.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}{quickCreate.isPending ? "Đang lưu…" : `Lưu ${quickEntries.length} Lô`}</Button></div>
          </form>
        </Panel>
        <Panel title="Import Excel" description="Dùng mẫu để nhập hàng loạt theo Tháng/Năm và Mã lô hoặc Tên lô. Dữ liệu lô được đối chiếu với hệ thống."><div className="grid gap-3"><div className="flex flex-col gap-2 sm:flex-row"><Button type="button" variant="outline" className="min-h-11 w-full sm:w-auto" onClick={downloadTemplate}><FileSpreadsheet className="mr-2 h-4 w-4" />Tải mẫu</Button><label className="inline-flex min-h-11 w-full cursor-pointer items-center justify-center rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-800 sm:w-auto"><UploadCloud className="mr-2 h-4 w-4" />Chọn tệp Excel<input className="sr-only" type="file" accept=".xlsx,.xls" onChange={readFile} /></label></div>{parsing ? <p className="flex items-center gap-2 rounded-lg bg-sky-50 p-3 text-sm text-sky-800"><Loader2 className="h-4 w-4 animate-spin" />Đang kiểm tra tệp…</p> : null}{fileName ? <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-3"><p className="font-semibold text-emerald-900">{fileName} · {importRows.length} dòng hợp lệ</p>{importLockedPeriod ? <p className="mt-2 rounded-lg bg-amber-50 p-2 text-xs font-medium text-amber-800">Kỳ {importLockedPeriod.replace("-", "/")} đã chốt. Không thể nhập tệp này.</p> : null}<div className="mt-2 hidden overflow-x-auto sm:block"><table className="w-full min-w-[500px] text-left text-xs"><thead><tr className="border-b border-emerald-100"><th className="p-2">Đội</th><th className="p-2">Lô</th><th className="p-2">Tháng/Năm</th><th className="p-2 text-right">Mủ đông/tạp</th><th className="p-2 text-right">Quy khô</th></tr></thead><tbody>{importRows.slice(0, 5).map((row, index) => { const plot = (plots ?? []).find(item => item.id === row.plotId); return <tr key={`${row.plotId}-${index}`}><td className="p-2">{plot?.unit}</td><td className="p-2">{plot?.name}</td><td className="p-2">{monthYearLabel(row.recordDate)}</td><td className="p-2 text-right">{formatQuantity(row.frozenContaminatedLatex)}</td><td className="p-2 text-right">{formatQuantity(row.dryRubber)}</td></tr>; })}</tbody></table></div><div className="mt-2 grid gap-2 sm:hidden">{importRows.slice(0, 5).map((row, index) => { const plot = (plots ?? []).find(item => item.id === row.plotId); return <article key={`mobile-${row.plotId}-${index}`} className="min-w-0 rounded-lg border border-emerald-100 bg-white p-3"><p className="break-words font-semibold text-emerald-900">{plot?.name ?? "—"}</p><p className="mt-1 break-words text-xs text-slate-500">{plot?.unit ?? "—"} · {monthYearLabel(row.recordDate)}</p><div className="mt-2 grid grid-cols-2 gap-2 text-xs"><span className="rounded-md bg-emerald-50 px-2 py-2 text-emerald-800">Mủ đông/tạp<br /><strong>{formatQuantity(row.frozenContaminatedLatex)} kg</strong></span><span className="rounded-md bg-sky-50 px-2 py-2 text-sky-800">Quy khô<br /><strong>{formatQuantity(row.dryRubber)} kg</strong></span></div></article>; })}</div><Button type="button" className="mt-3 min-h-11 w-full bg-emerald-700 hover:bg-emerald-800" disabled={importProduction.isPending || Boolean(importLockedPeriod)} onClick={() => importProduction.mutate({ rows: importRows })}>{importProduction.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <UploadCloud className="mr-2 h-4 w-4" />}{importProduction.isPending ? "Đang nhập…" : `Nhập ${importRows.length} dòng`}</Button></div> : null}{issues.length ? <div className="rounded-lg bg-amber-50 p-3 text-xs text-amber-900"><p className="font-bold">{issues.length} dòng không hợp lệ</p>{issues.slice(0, 3).map(issue => <p key={issue} className="mt-1">• {issue}</p>)}</div> : null}</div></Panel>
      </div>
      <Panel title="Tổng hợp sản lượng theo lô" description="Theo mẫu Đội, Lô, Năm trồng, Diện tích, Mủ đông/tạp và Quy khô.">
        <div className="mb-4 grid gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3 sm:grid-cols-3"><Field label="Đội"><select value={unit} onChange={event => setUnit(event.target.value)} className="min-h-11 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="all">Tất cả Đội</option>{units.map(item => <option key={item} value={item}>{item}</option>)}</select></Field><Field label="Năm"><select value={year} onChange={event => { setYear(event.target.value); setMonth("all"); }} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="all">Tất cả năm</option>{years.map(item => <option key={item} value={item}>Năm {item}</option>)}</select></Field><Field label="Tháng"><select value={month} onChange={event => setMonth(event.target.value)} disabled={year === "all"} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm disabled:opacity-50"><option value="all">Tất cả tháng</option>{Array.from({ length: 12 }, (_, index) => index + 1).map(item => <option key={item} value={item}>Tháng {item}</option>)}</select></Field></div>
        <div className="mb-4 grid gap-3 md:grid-cols-2"><ComparisonCard title="Mủ đông, tạp — tăng/giảm" current={productionComparison.current.frozen} currentLabel={productionComparison.current.monthLabel} previous={productionComparison.previousMonth?.frozen ?? null} previousLabel={productionComparison.previousMonth?.monthLabel} emptyLabel="Chưa có tháng trước có dữ liệu" /><ComparisonCard title="Quy khô — tăng/giảm" current={productionComparison.current.dry} currentLabel={productionComparison.current.monthLabel} previous={productionComparison.previousMonth?.dry ?? null} previousLabel={productionComparison.previousMonth?.monthLabel} emptyLabel="Chưa có tháng trước có dữ liệu" /></div>
        {productionComparison.previousYear ? <><p className="mb-2 text-sm font-semibold text-slate-700">So với cùng kỳ</p><div className="mb-4 grid gap-3 md:grid-cols-2"><ComparisonCard title="Mủ đông, tạp — cùng kỳ" current={productionComparison.current.frozen} currentLabel={productionComparison.current.monthLabel} previous={productionComparison.previousYear.frozen} previousLabel={productionComparison.previousYear.monthLabel} emptyLabel="Chưa có cùng kỳ có dữ liệu" /><ComparisonCard title="Quy khô — cùng kỳ" current={productionComparison.current.dry} currentLabel={productionComparison.current.monthLabel} previous={productionComparison.previousYear.dry} previousLabel={productionComparison.previousYear.monthLabel} emptyLabel="Chưa có cùng kỳ có dữ liệu" /></div></> : null}
        <div className="mb-4"><p className="mb-2 text-sm font-semibold text-slate-700">So sánh sản lượng theo Đội</p>{teamProductionRows.length ? <ChartContainer config={teamProductionConfig} className="h-64 w-full min-w-0"><BarChart data={teamProductionRows} barGap={6}><CartesianGrid vertical={false} /><XAxis dataKey="unit" tickLine={false} axisLine={false} interval={0} tick={{ fontSize: 11 }} tickMargin={8} /><YAxis tickFormatter={formatQuantity} width={62} /><ChartTooltip content={<ChartTooltipContent formatter={(value) => `${formatQuantity(Number(value))} kg`} />} /><Bar dataKey="frozenContaminatedLatex" name="frozenContaminatedLatex" fill="var(--color-frozenContaminatedLatex)" radius={[4, 4, 0, 0]} /><Bar dataKey="dryRubber" name="dryRubber" fill="var(--color-dryRubber)" radius={[4, 4, 0, 0]} /></BarChart></ChartContainer> : <div className="grid h-40 place-items-center rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 text-center text-sm text-slate-500">Chưa có dữ liệu sản lượng theo lô trong phạm vi Tháng/Năm và Đội đang chọn.</div>}</div>
        {isLoading ? <div className="h-56 animate-pulse rounded-xl bg-slate-100" /> : !reportRows.length ? <EmptyState title="Chưa có sản lượng theo lô" description="Nhập tay, nhập nhanh hoặc import Excel theo Tháng/Năm và đúng tên Lô để hình thành tổng hợp. Hệ thống không phân bổ số liệu Đội hiện có sang Lô khi chưa có dữ liệu nguồn." /> : <><div className="grid gap-3 md:hidden">{reportRows.map((row, index) => <article key={`mobile-report-${row.plotId}`} className="min-w-0 rounded-xl border border-slate-200 bg-white p-3"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="break-words text-xs font-semibold text-slate-500">#{index + 1} · {row.unit}</p><p className="break-words font-semibold text-slate-900">{row.plotName}</p><p className="break-words text-xs text-slate-500">{row.plotCode}</p></div><span className="shrink-0 text-xs text-slate-500">{row.plantedYear ?? "—"}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-sm"><div className="rounded-lg bg-slate-50 px-3 py-2"><p className="text-xs text-slate-500">Diện tích</p><p className="font-semibold text-slate-800">{formatAreaHa(row.areaHa)} ha</p></div><div className="rounded-lg bg-emerald-50 px-3 py-2"><p className="text-xs text-emerald-700">Mủ đông, tạp</p><p className="font-semibold text-emerald-700">{formatQuantity(row.frozenContaminatedLatex)} kg</p></div><div className="col-span-2 rounded-lg bg-sky-50 px-3 py-2"><p className="text-xs text-sky-700">Quy khô</p><p className="font-semibold text-sky-700">{formatQuantity(row.dryRubber)} kg</p></div></div></article>)}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[820px] text-left"><thead><tr className="border-b border-slate-200 text-xs font-bold uppercase tracking-[0.1em] text-slate-400"><th className="px-3 py-3">STT</th><th className="px-3 py-3">Đội</th><th className="px-3 py-3">Lô</th><th className="px-3 py-3 text-right">Năm trồng</th><th className="px-3 py-3 text-right">Diện tích (ha)</th><th className="px-3 py-3 text-right">Mủ đông, tạp</th><th className="px-3 py-3 text-right">Quy khô</th></tr></thead><tbody>{reportRows.map((row, index) => <tr key={row.plotId} className="border-b border-slate-100 last:border-0"><td className="px-3 py-4 text-slate-500">{index + 1}</td><td className="px-3 py-4 font-semibold text-slate-700">{row.unit}</td><td className="px-3 py-4"><p className="font-semibold text-slate-800">{row.plotName}</p><p className="text-xs text-slate-500">{row.plotCode}</p></td><td className="px-3 py-4 text-right">{row.plantedYear ?? "—"}</td><td className="px-3 py-4 text-right">{formatAreaHa(row.areaHa)}</td><td className="px-3 py-4 text-right font-semibold text-emerald-700">{formatQuantity(row.frozenContaminatedLatex)} kg</td><td className="px-3 py-4 text-right font-semibold text-sky-700">{formatQuantity(row.dryRubber)} kg</td></tr>)}</tbody><tfoot><tr className="bg-slate-50 font-bold text-slate-900"><td colSpan={5} className="px-3 py-4">Tổng khối lượng</td><td className="px-3 py-4 text-right text-emerald-700">{formatQuantity(totals.frozen)} kg</td><td className="px-3 py-4 text-right text-sky-700">{formatQuantity(totals.dry)} kg</td></tr></tfoot></table></div></>}
      </Panel>
    </div>
  </div>;
}
...
function ComparisonCard({ title, current, currentLabel, previous, previousLabel, emptyLabel }: { title: string; current: number; currentLabel?: string; previous: number | null; previousLabel?: string; emptyLabel: string }) { const change = previous == null ? null : current - previous; const percent = previous && previous !== 0 ? (change! / previous) * 100 : null; return <div className="rounded-xl border border-slate-100 bg-slate-50 p-3"><p className="text-xs font-semibold text-slate-600">{title}</p><div className="mt-2 grid gap-1"><div className="flex items-baseline justify-between gap-3"><span className="text-xs font-medium text-slate-500">Hiện tại</span><span className="text-xs font-medium text-slate-500">{currentLabel ?? "—"}</span></div><p className="flex items-baseline justify-between gap-3 text-lg font-bold text-slate-900"><span>{formatQuantity(current)} kg</span><span className="sr-only">{currentLabel ?? ""}</span></p>{previous == null ? <p className="mt-1 text-sm font-medium text-slate-500">{emptyLabel}</p> : <><div className="mt-2 flex items-baseline justify-between gap-3"><span className="text-xs font-medium text-slate-500">Cùng kỳ/đối chiếu</span><span className="text-xs font-medium text-slate-500">{previousLabel ?? "—"}</span></div><p className="flex items-baseline justify-between gap-3 text-base font-semibold text-slate-700"><span>{formatQuantity(previous)} kg</span><span className={`text-sm ${(change ?? 0) >= 0 ? "text-emerald-700" : "text-rose-700"}`}>{(change ?? 0) >= 0 ? "Tăng" : "Giảm"} {formatQuantity(Math.abs(change ?? 0))} kg{percent == null ? "" : ` (${formatQuantity(Math.abs(percent))}%)`}</span></p></>}</div></div>; }
function PeriodLockNotice({ year, month, locked, isAdmin, pending, onLock, onUnlock }: { year: string; month: string; locked: boolean; isAdmin: boolean; pending: boolean; onLock: () => void; onUnlock: () => void }) { return <div className={`flex min-w-0 flex-col items-stretch gap-2 rounded-xl sm:flex-row sm:items-center sm:justify-between border px-3 py-2 text-sm ${locked ? "border-amber-200 bg-amber-50 text-amber-900" : "border-slate-200 bg-slate-50 text-slate-700"}`}><span className="flex items-center gap-2 font-medium">{locked ? <LockKeyhole className="h-4 w-4" /> : <LockKeyholeOpen className="h-4 w-4" />}{locked ? `Tháng ${month}/${year} đã chốt` : `Tháng ${month}/${year} chưa chốt`}</span>{isAdmin ? <Button type="button" size="sm" variant={locked ? "outline" : "default"} className={`${locked ? "bg-white" : "bg-emerald-700 hover:bg-emerald-800"} min-h-11 w-full sm:w-auto`} disabled={pending} onClick={locked ? onUnlock : onLock}>{pending ? "Đang xử lý…" : locked ? "Mở khóa" : "Chốt tháng"}</Button> : null}</div>; }
