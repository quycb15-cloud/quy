  ...
  return <div className="page-enter">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[0.92fr_1.45fr]">
      ...
      <Panel title="Tổng hợp sản lượng theo lô" description="Theo mẫu Đội, Lô, Năm trồng, Diện tích, Mủ đông/tạp và Quy khô.">
        ...
        <div className="mb-4 grid grid-cols-2 gap-3"><Summary label="Mủ đông, tạp" value={productionComparison.current.frozen} /><Summary label="Quy khô" value={productionComparison.current.dry} /></div>
        {productionComparison.previousMonth || productionComparison.previousYear ? <div className="mb-4 grid gap-3 md:grid-cols-2"><ComparisonCard title="So với tháng trước" current={productionComparison.current.total} previous={productionComparison.previousMonth?.total ?? null} /><ComparisonCard title="So với cùng kỳ năm trước" current={productionComparison.current.total} previous={productionComparison.previousYear?.total ?? null} /></div> : null}
...
function ComparisonCard({ title, current, previous }: { title: string; current: number; previous: number | null }) { const change = previous == null ? null : current - previous; const percent = previous && previous !== 0 ? (change! / previous) * 100 : null; return <div className="rounded-xl border border-slate-100 bg-slate-50 p-3"><p className="text-xs font-semibold text-slate-600">{title}</p><p className="mt-1 text-lg font-bold text-slate-900">Hiện tại: {formatQuantity(current)} kg</p><p className={`mt-1 text-sm font-semibold ${change == null || change >= 0 ? "text-emerald-700" : "text-rose-700"}`}>{change == null ? "Chưa có dữ liệu đối chiếu" : `${change >= 0 ? "Tăng" : "Giảm"} ${formatQuantity(Math.abs(change))} kg${percent == null ? "" : ` (${formatQuantity(Math.abs(percent))}%)`}`}</p></div>; }
