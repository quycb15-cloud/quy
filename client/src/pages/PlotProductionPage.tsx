...
function ComparisonCard({ title, current, currentLabel, previous, previousLabel, emptyLabel }: { title: string; current: number; currentLabel?: string; previous: number | null; previousLabel?: string; emptyLabel: string }) { const change = previous == null ? null : current - previous; const percent = previous && previous !== 0 ? (change! / previous) * 100 : null; return <div className="rounded-xl border border-slate-100 bg-slate-50 p-3"><p className="text-xs font-semibold text-slate-600">{title}</p><div className="mt-2 flex items-baseline justify-between gap-3"><p className="text-lg font-bold text-slate-900">{formatQuantity(current)} kg</p><span className="text-xs font-medium text-slate-500">{currentLabel ?? "—"}</span></div><p className={`mt-1 flex items-baseline justify-between gap-3 text-sm font-semibold ${change == null || change >= 0 ? "text-emerald-700" : "text-rose-700"}`}>{change == null ? emptyLabel : <><span>{change >= 0 ? "Tăng" : "Giảm"} {formatQuantity(Math.abs(change))} kg{percent == null ? "" : ` (${formatQuantity(Math.abs(percent))}%)`}</span><span className="text-xs font-medium text-slate-500">{previousLabel ?? "—"}</span></>}</p></div>; }
