...
import { summarizeCareDaily, dailyCompletionPercent, cumulativeCompletionPercent, summarizeCareByDateAndWorkContent } from "@/lib/careDailySummary";
...
export default function CareOperationsPage() {
  ...
  const contentSummaries = useMemo(() => summarizeCareByDateAndWorkContent(filteredRecords), [filteredRecords]);
  const contentSummaryByKey = useMemo(() => new Map(contentSummaries.map(summary => [`${summary.dateKey}|${summary.workContent.toLocaleLowerCase("vi-VN")}`, summary])), [contentSummaries]);
  ...
  const mobileRecordCards = <div className="space-y-2 md:hidden">{orderedRecords.map((row, index) => {
    const nextRow = orderedRecords[index + 1];
    const rowKey = `${careDateKey(row.activityDate)}|${(row.workContent?.trim() || "Tất cả nội dung").toLocaleLowerCase("vi-VN")}`;
    const nextKey = nextRow ? `${careDateKey(nextRow.activityDate)}|${(nextRow.workContent?.trim() || "Tất cả nội dung").toLocaleLowerCase("vi-VN")}` : "";
    const groupSummary = contentSummaryByKey.get(rowKey);
    const isLastInContentGroup = !isTapping && rowKey !== nextKey;
    return <Fragment key={`mobile-${row.id}`}>
      <article className="min-w-0 rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
        <div className="flex min-w-0 items-start justify-between gap-2"><div className="min-w-0"><p className="break-words font-semibold text-slate-900">{row.unit}{row.gardenName ? ` · ${row.gardenName}` : ""}</p><p className="text-xs text-slate-500">{formatCareDate(row.activityDate)}</p></div><Button type="button" variant="outline" className="min-h-11 shrink-0" onClick={() => loadRecord(row)}><Pencil className="mr-1 h-3 w-3" />Sửa</Button></div>
        {needsWorkContent ? <p className="mt-2 break-words rounded-md bg-slate-50 px-2 py-1.5 text-sm font-medium text-slate-700">{row.workContent || "—"}</p> : null}
        <dl className="mt-3 grid min-w-0 grid-cols-2 gap-x-3 gap-y-2 text-sm">
          <div><dt className="text-xs text-slate-500">KH</dt><dd className="font-semibold">{displayQuantity(row.planQuantity)} {config[category].metric}</dd></div>
          <div><dt className="text-xs text-slate-500">{isTapping ? "Cạo xong" : "TH"}</dt><dd className="font-semibold">{displayQuantity(row.actualQuantity)} {config[category].metric}</dd></div>
          {isTapping ? <><div><dt className="text-xs text-slate-500">Chưa cạo</dt><dd>{displayQuantity(row.pendingGardens)}</dd></div><div><dt className="text-xs text-slate-500">Cạo chưa xong</dt><dd>{displayQuantity(row.partialGardens)}</dd></div><div><dt className="text-xs text-slate-500">% hoàn thành</dt><dd className="font-semibold text-emerald-700">{row.progressPercent ? formatPercent(row.progressPercent) : "—"}</dd></div><div><dt className="text-xs text-slate-500">Cạo tiếp vườn</dt><dd className="break-words">{row.nextGarden ?? "—"}</dd></div><div><dt className="text-xs text-slate-500">KH tiếp</dt><dd>{displayQuantity(row.nextGardenPlanQuantity)}</dd></div><div><dt className="text-xs text-slate-500">TH tiếp</dt><dd>{displayQuantity(row.nextGardenActualQuantity)}</dd></div></> : null}
          {!isTapping ? <><div><dt className="text-xs text-slate-500">Lũy kế</dt><dd>{displayQuantity(row.cumulativeQuantity)} {config[category].metric}</dd></div><div><dt className="text-xs text-slate-500">% hoàn thành</dt><dd className="font-semibold text-emerald-700">{row.progressPercent ? formatPercent(row.progressPercent) : "—"}</dd></div></> : null}
        </dl>
      </article>
      {isLastInContentGroup && groupSummary ? <article className="min-w-0 rounded-lg border border-sky-200 bg-sky-50/70 p-3 text-sm text-sky-900"><p className="font-semibold">Tổng ngày {formatCareDate(groupSummary.dateKey)}</p>{needsWorkContent ? <p className="mt-1 break-words">{groupSummary.workContent}</p> : null}<dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2"><div><dt className="text-xs text-sky-700">KH</dt><dd className="font-semibold">{displayQuantity(groupSummary.plan)}</dd></div><div><dt className="text-xs text-sky-700">TH</dt><dd className="font-semibold">{displayQuantity(groupSummary.actual)}</dd></div><div><dt className="text-xs text-sky-700">Lũy kế</dt><dd>{displayQuantity(groupSummary.cumulative)}</dd></div><div><dt className="text-xs text-sky-700">% hoàn thành</dt><dd className="font-semibold">{groupSummary.plan > 0 ? formatPercent(cumulativeCompletionPercent(groupSummary)) : "—"}</dd></div></dl></article> : null}
    </Fragment>;
  })}{orderedRecords.length ? <article className="min-w-0 rounded-lg border-2 border-emerald-200 bg-emerald-50/70 p-3 text-sm text-slate-800"><p className="font-semibold">Tổng cộng</p>{needsWorkContent ? <p className="mt-1 break-words">Tất cả nội dung</p> : null}<dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2">{isTapping ? <><div><dt className="text-xs text-slate-600">KH</dt><dd>{displayQuantity(dailySummary.plan)}</dd></div><div><dt className="text-xs text-slate-600">Cạo xong</dt><dd>{displayQuantity(dailySummary.actual)}</dd></div><div><dt className="text-xs text-slate-600">Chưa cạo</dt><dd>{displayQuantity(dailySummary.pending)}</dd></div><div><dt className="text-xs text-slate-600">Cạo chưa xong</dt><dd>{displayQuantity(dailySummary.partial)}</dd></div><div><dt className="text-xs text-slate-600">% hoàn thành</dt><dd className="font-semibold text-emerald-700">{dailySummary.plan > 0 ? formatPercent(dailyCompletionPercent(dailySummary)) : "—"}</dd></div><div><dt className="text-xs text-slate-600">KH tiếp</dt><dd>{displayQuantity(dailySummary.nextPlan)}</dd></div><div><dt className="text-xs text-slate-600">TH tiếp</dt><dd>{displayQuantity(dailySummary.nextActual)}</dd></div></> : null}{!isTapping ? <><div><dt className="text-xs text-slate-600">KH</dt><dd>{displayQuantity(dailySummary.plan)}</dd></div><div><dt className="text-xs text-slate-600">TH</dt><dd>{displayQuantity(dailySummary.actual)}</dd></div><div><dt className="text-xs text-slate-600">Lũy kế</dt><dd>{displayQuantity(dailySummary.cumulative)}</dd></div><div><dt className="text-xs text-slate-600">% hoàn thành</dt><dd className="font-semibold text-emerald-700">{dailySummary.plan > 0 ? formatPercent(cumulativeCompletionPercent(dailySummary)) : "—"}</dd></div></> : null}</dl></article> : null}</div>;

  const mobileMonthlyCards = <div className="space-y-2 md:hidden">{monthTappingRows.map(row => <article key={`mobile-${row.unit}|${row.gardenName}`} className="min-w-0 rounded-lg border border-emerald-100 bg-emerald-50/40 p-3"><div className="flex min-w-0 items-start justify-between gap-2"><p className="min-w-0 break-words font-semibold text-slate-900">{row.unit} · {row.gardenName}</p><span className="shrink-0 text-sm font-semibold text-emerald-700">{row.plan > 0 ? formatPercent(monthlyCompletionPercent(row)) : "—"}</span></div><dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2 text-sm"><div><dt className="text-xs text-slate-500">KH</dt><dd>{displayQuantity(row.plan)}</dd></div><div><dt className="text-xs text-slate-500">Cạo xong</dt><dd>{displayQuantity(row.actual)}</dd></div><div><dt className="text-xs text-slate-500">Chưa cạo</dt><dd>{displayQuantity(row.pending)}</dd></div><div><dt className="text-xs text-slate-500">Cạo chưa xong</dt><dd>{displayQuantity(row.partial)}</dd></div></dl></article>)}</div>;

  return <div className="page-enter min-w-0">
    <PageHeader eyebrow="Vận hành vườn" title="Khai thác & chăm sóc hằng ngày" description="Năm bảng theo mẫu: cạo mủ theo Vườn A/B/C; rập thiết kế, trang bị; chăm sóc; phun, bôi thuốc; bón phân." action={<div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap sm:justify-end"><Button variant="outline" className="min-h-11 w-full justify-center bg-white sm:w-auto" onClick={downloadCareTemplate}><FileSpreadsheet className="mr-2 h-4 w-4" />Tải mẫu Excel</Button><label className="inline-flex min-h-11 w-full cursor-pointer items-center justify-center rounded-md bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-800 sm:w-auto"><UploadCloud className="mr-2 h-4 w-4" />Chọn tệp Excel<input ref={importInputRef} className="sr-only" type="file" accept=".xlsx,.xls" onChange={importCareFile} /></label><Button variant="outline" className="min-h-11 w-full justify-center bg-white sm:w-auto" onClick={exportWorkbook}><Download className="mr-2 h-4 w-4" />Xuất tổng hợp Excel</Button></div>} />
    <div className="mt-4 grid gap-3 lg:grid-cols-[18rem_minmax(0,1fr)] xl:grid-cols-[20rem_minmax(0,1fr)]">
      <Panel compact className="min-w-0" title="Nhập dữ liệu hằng ngày" description={config[category].description}>
        <div className="grid min-w-0 gap-2 text-xs [&_input]:min-h-11 [&_input]:h-10 [&_input]:text-xs [&_label]:text-[11px] [&_select]:h-9 [&_select]:text-xs [&_textarea]:min-h-16 [&_textarea]:text-xs">
          <Field label="Bảng theo dõi"><Select value={category} onValueChange={value => { setCategory(value as Category); setForm(blankForm()); }}><SelectTrigger className="min-h-11"><SelectValue /></SelectTrigger><SelectContent>{categories.map(key => <SelectItem key={key} value={key}>{config[key].label}</SelectItem>)}</SelectContent></Select></Field>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2"><Field label="Ngày"><Input type="date" value={form.date} onChange={e => update("date", e.target.value)} /></Field><Field label="Đội"><select value={form.unit} onChange={e => { const team = e.target.value; update("unit", team); update("gardenName", ""); writeLastSelectedCareTeam(team, window.localStorage); }} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"><option value="">{teamOptions.length ? "Chọn Đội" : "Chưa có Đội trong phạm vi"}</option>{teamOptions.map(team => <option key={team} value={team}>{team}</option>)}</select></Field></div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap"><Button className="min-h-11 w-full justify-center bg-emerald-700 sm:w-auto hover:bg-emerald-800" onClick={submit} disabled={save.isPending}><Save className="mr-2 h-4 w-4" />{save.isPending ? "Đang lưu…" : editingKey ? "Cập nhật dữ liệu ngày" : "Lưu dữ liệu hằng ngày"}</Button>{editingKey ? <><Button type="button" variant="outline" className="min-h-11 w-full justify-center border-red-200 text-red-700 hover:bg-red-50 sm:w-auto" onClick={confirmRemove} disabled={remove.isPending}><Trash2 className="mr-2 h-4 w-4" />{remove.isPending ? "Đang xóa…" : "Xóa dữ liệu"}</Button><Button type="button" variant="outline" className="min-h-11 w-full justify-center sm:w-auto" onClick={() => { setEditingKey(null); setEditingId(null); setForm({ ...blankForm(), unit: form.unit }); }}><X className="mr-2 h-4 w-4" />Hủy sửa</Button></> : null}</div>
        </div>
      </Panel>
      <Panel compact className="min-w-0" title={config[category].label} description={`KH ${formatQuantity(totals.plan)} · TH ${formatQuantity(totals.actual)} ${config[category].metric}`}><div className="mb-2 grid min-w-0 gap-2 rounded-lg border border-slate-200 bg-slate-50 p-2 sm:grid-cols-[1fr_1fr_auto_auto]"><Field label="Từ ngày"><Input type="date" value={fromDate} max={toDate || undefined} onChange={event => setFromDate(event.target.value)} /></Field><Field label="Đến ngày"><Input type="date" value={toDate} min={fromDate || undefined} onChange={event => setToDate(event.target.value)} /></Field><Button type="button" variant="outline" className="min-h-11 w-full justify-center self-end sm:w-auto" disabled={!fromDate && !toDate} onClick={() => { const latest = latestCareDate(records); setFromDate(latest ?? ""); setToDate(latest ?? ""); }}>Đặt lại</Button><Button type="button" variant="outline" className="min-h-11 w-full justify-center self-end sm:w-auto" onClick={exportCurrentBoard}><Download className="mr-2 h-4 w-4" />Xuất Excel bảng này</Button></div>            <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[650px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="p-2">Ngày</th><th className="p-2">Đội / Vườn</th>{needsWorkContent ? <th className="p-2">Nội dung công việc</th> : null}<th className="p-2 text-right">KH</th><th className="p-2 text-right">{isTapping ? "Cạo xong" : "TH"}</th>{isTapping ? <><th className="p-2 text-right">Chưa cạo</th><th className="p-2 text-right">Cạo chưa xong</th><th className="p-2 text-right">% hoàn thành</th><th className="p-2">Cạo tiếp vườn</th><th className="p-2 text-right">KH tiếp</th><th className="p-2 text-right">TH tiếp</th></> : <><th className="p-2 text-right">Lũy kế</th><th className="p-2 text-right">% hoàn thành</th></>}<th className="p-2">Thao tác</th></tr></thead><tbody>{orderedRecords.map((row, index) => { const nextRow = orderedRecords[index + 1]; const rowKey = `${careDateKey(row.activityDate)}|${(row.workContent?.trim() || "Tất cả nội dung").toLocaleLowerCase("vi-VN")}`; const nextKey = nextRow ? `${careDateKey(nextRow.activityDate)}|${(nextRow.workContent?.trim() || "Tất cả nội dung").toLocaleLowerCase("vi-VN")}` : ""; const groupSummary = contentSummaryByKey.get(rowKey); const isLastInContentGroup = !isTapping && rowKey !== nextKey; return <Fragment key={row.id}><tr className="border-b border-slate-100"><td className="p-2">{formatCareDate(row.activityDate)}</td><td className="p-2"><b>{row.unit}</b><span className="block text-xs text-slate-500">{row.gardenName}</span></td>{needsWorkContent ? <td className="p-2 font-medium text-slate-700">{row.workContent || "—"}</td> : null}<td className="p-2 text-right">{displayQuantity(row.planQuantity)}</td><td className="p-2 text-right">{displayQuantity(row.actualQuantity)}</td>{isTapping ? <><td className="p-2 text-right">{displayQuantity(row.pendingGardens)}</td><td className="p-2 text-right">{displayQuantity(row.partialGardens)}</td><td className="p-2 text-right font-semibold text-emerald-700">{row.progressPercent ? formatPercent(row.progressPercent) : "—"}</td><td className="p-2">{row.nextGarden ?? "—"}</td><td className="p-2 text-right">{displayQuantity(row.nextGardenPlanQuantity)}</td><td className="p-2 text-right">{displayQuantity(row.nextGardenActualQuantity)}</td></> : <><td className="p-2 text-right">{displayQuantity(row.cumulativeQuantity)}</td><td className="p-2 text-right font-semibold text-emerald-700">{row.progressPercent ? formatPercent(row.progressPercent) : "—"}</td></>}<td className="p-2"><Button type="button" variant="outline" size="sm" className="min-h-11" onClick={() => loadRecord(row)}><Pencil className="mr-1 h-3 w-3" />Sửa</Button></td></tr>{isLastInContentGroup && groupSummary ? <tr className="border-b border-sky-200 bg-sky-50/70 font-semibold text-sky-900"><td className="p-2" colSpan={2}>Tổng ngày {formatCareDate(groupSummary.dateKey)}</td>{needsWorkContent ? <td className="p-2">{groupSummary.workContent}</td> : null}<td className="p-2 text-right">{displayQuantity(groupSummary.plan)}</td><td className="p-2 text-right">{displayQuantity(groupSummary.actual)}</td><td className="p-2 text-right">{displayQuantity(groupSummary.cumulative)}</td><td className="p-2 text-right text-sky-700">{groupSummary.plan > 0 ? formatPercent(cumulativeCompletionPercent(groupSummary)) : "—"}</td><td className="p-2">—</td></tr> : null}</Fragment>; })}{orderedRecords.length ? <tr className="border-t-2 border-emerald-200 bg-emerald-50/70 font-semibold text-slate-800"><td className="p-2" colSpan={2}>Tổng cộng</td>{needsWorkContent ? <td className="p-2">Tất cả nội dung</td> : null}<td className="p-2 text-right">{displayQuantity(dailySummary.plan)}</td><td className="p-2 text-right">{displayQuantity(dailySummary.actual)}</td>{isTapping ? <><td className="p-2 text-right">{displayQuantity(dailySummary.pending)}</td><td className="p-2 text-right">{displayQuantity(dailySummary.partial)}</td><td className="p-2 text-right text-emerald-700">{dailySummary.plan > 0 ? formatPercent(dailyCompletionPercent(dailySummary)) : "—"}</td><td className="p-2">—</td><td className="p-2 text-right">{displayQuantity(dailySummary.nextPlan)}</td><td className="p-2 text-right">{displayQuantity(dailySummary.nextActual)}</td></> : <><td className="p-2 text-right">{displayQuantity(dailySummary.cumulative)}</td><td className="p-2 text-right text-emerald-700">{dailySummary.plan > 0 ? formatPercent(cumulativeCompletionPercent(dailySummary)) : "—"}</td></>}<td className="p-2">—</td></tr> : null}</tbody></table></div>{mobileRecordCards}{!records.length && !isLoading ? <p className="py-12 text-center text-sm text-slate-500">Chưa có dữ liệu cho bảng này.</p> : records.length && !orderedRecords.length ? <p className="py-12 text-center text-sm text-slate-500">Không có dữ liệu trong khoảng thời gian đã chọn.</p> : null}      </Panel>
      <div className="w-full min-w-0 lg:col-start-2 rounded-xl border border-slate-200 bg-white p-3 shadow-sm"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-semibold text-slate-900">Ghi chú</h3><p className="text-xs text-slate-500">Ghi chú đã nhập theo ngày và Đội; Đội không có ghi chú không hiển thị.</p></div><Button type="button" variant="outline" size="sm" className="min-h-11" onClick={openNoteComposer} disabled={!orderedRecords.length}><Save className="mr-1 h-3 w-3" />Thêm Ghi chú</Button></div>{noteRows.length ? <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 text-sm"><div className="mb-1 font-semibold text-slate-700">Ngày {noteRows[0].date}</div>{noteRows.map(row => <div key={`${row.date}|${row.unit}`} className="border-b border-slate-200 py-1 last:border-b-0"><span className="font-semibold">{row.unit}:</span> <span className="break-words">{row.lines.join("; ")}</span></div>)}</div> : <p className="rounded-lg border border-dashed border-slate-300 p-3 text-sm text-slate-500">Chưa có ghi chú trong phạm vi ngày đang chọn.</p>}{noteComposerOpen && noteTarget ? <div className="mt-2 grid min-w-0 grid-cols-1 gap-2 rounded-lg border border-emerald-200 bg-emerald-50/50 p-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]"><div><p className="mb-1 text-xs text-slate-600">{formatCareDate(noteTarget.activityDate)} · {noteTarget.unit} · {noteTarget.workContent || config[noteTarget.category].label}</p><Textarea value={noteDraft} onChange={event => setNoteDraft(event.target.value)} placeholder="Nhập ghi chú" /></div><Button type="button" variant="outline" className="min-h-11 w-full justify-center self-end sm:w-auto" onClick={submitNoteComposer} disabled={save.isPending}><Save className="mr-1 h-3 w-3" />Lưu</Button><Button type="button" variant="outline" className="min-h-11 w-full justify-center self-end sm:w-auto" onClick={() => setNoteComposerOpen(false)}>Hủy</Button></div> : null}</div>
      {isTapping ? <Panel compact className="w-full min-w-0 lg:col-span-2" title="Tổng hợp cạo mủ trong tháng" description="Cộng dồn từ đầu tháng đến hiện tại theo từng Đội và Vườn A/B/C."><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[680px] table-fixed text-xs sm:text-sm"><thead className="bg-emerald-50 text-left text-xs uppercase text-emerald-800"><tr><th className="p-2">Đội</th><th className="p-2">Vườn</th><th className="p-2 text-right">KH</th><th className="p-2 text-right">Cạo xong</th><th className="p-2 text-right">Chưa cạo</th><th className="p-2 text-right">Cạo chưa xong</th><th className="p-2 text-right">% hoàn thành</th></tr></thead><tbody>{monthTappingRows.map(row => <tr key={`${row.unit}|${row.gardenName}`} className="border-b border-slate-100"><td className="p-2 font-semibold">{row.unit}</td><td className="p-2">{row.gardenName}</td><td className="p-2 text-right">{displayQuantity(row.plan)}</td><td className="p-2 text-right">{displayQuantity(row.actual)}</td><td className="p-2 text-right">{displayQuantity(row.pending)}</td><td className="p-2 text-right">{displayQuantity(row.partial)}</td><td className="p-2 text-right font-semibold text-emerald-700">{row.plan > 0 ? formatPercent(monthlyCompletionPercent(row)) : "—"}</td></tr>)}</tbody></table></div>{mobileMonthlyCards}{!monthTappingRows.length ? <p className="py-8 text-center text-sm text-slate-500">Chưa có dữ liệu cạo mủ từ đầu tháng.</p> : null}</Panel> : null}
    </div>
  </div>;
}
