...
export default function CareOperationsPage() {
  ...
  const save = trpc.operations.save.useMutation({
      onSuccess: async () => { await utils.operations.list.invalidate(); toast.success(editingKey ? "Đã cập nhật dữ liệu theo ngày" : `Đã lưu ${config[category].label.toLowerCase()}`); setEditingKey(null); setForm(value => ({ ...value, actual: "", cumulative: "", pending: "", partial: "", nextGarden: "", nextGardenPlan: "", nextGardenActual: "", note: "" })); },
  ...
  const monthTappingRows = useMemo(() => {
    if (!isTapping) return [];
    const grouped = new Map<string, { unit: string; gardenName: string; plan: number; actual: number; pending: number; partial: number; nextGarden: string; nextPlan: number; nextActual: number }>();
    records.filter(row => { const date = new Date(row.activityDate); return date >= monthStart && date <= monthEnd; }).forEach(row => {
      const key = `${row.unit}|${row.gardenName}`;
      const current = grouped.get(key) ?? { unit: row.unit, gardenName: row.gardenName ?? "", plan: 0, actual: 0, pending: 0, partial: 0, nextGarden: "", nextPlan: 0, nextActual: 0 };
      current.plan += row.planQuantity; current.actual += row.actualQuantity; current.pending += row.pendingGardens ?? 0; current.partial += row.partialGardens ?? 0; current.nextGarden = row.nextGarden ?? current.nextGarden; current.nextPlan += row.nextGardenPlanQuantity ?? 0; current.nextActual += row.nextGardenActualQuantity ?? 0; grouped.set(key, current);
    });
    return Array.from(grouped.values()).sort((left, right) => compareTeamName(left.unit, right.unit) || left.gardenName.localeCompare(right.gardenName, "vi"));
  }, [isTapping, records, monthStart.getTime(), monthEnd.getTime()]);
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
      <Panel title="Nhập dữ liệu hằng ngày" description={config[category].description}>
        <div className="grid gap-3">
          ...
          {isTapping ? <><div className="grid grid-cols-2 gap-3"><Field label="Chưa cạo"><Input inputMode="numeric" value={form.pending} onChange={e => update("pending", e.target.value)} /></Field><Field label="Cạo chưa xong"><Input inputMode="numeric" value={form.partial} onChange={e => update("partial", e.target.value)} /></Field></div><div className="rounded-xl border border-amber-100 bg-amber-50/60 p-3"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-amber-800">Cạo tiếp vườn</p><div className="grid gap-3 sm:grid-cols-3"><Field label="Tên vườn"><Input value={form.nextGarden} onChange={e => update("nextGarden", e.target.value)} placeholder="Tên vườn tiếp theo" /></Field><Field label="KH (Vườn)"><Input inputMode="numeric" value={form.nextGardenPlan} onChange={e => update("nextGardenPlan", e.target.value)} /></Field><Field label="TH (Vườn)"><Input inputMode="numeric" value={form.nextGardenActual} onChange={e => update("nextGardenActual", e.target.value)} /></Field></div></div></> : null}
          ...
          <div className="flex flex-wrap gap-2"><Button className="bg-emerald-700 hover:bg-emerald-800" onClick={submit} disabled={save.isPending}><Save className="mr-2 h-4 w-4" />{save.isPending ? "Đang lưu…" : editingKey ? "Cập nhật dữ liệu ngày" : "Lưu dữ liệu hằng ngày"}</Button>{editingKey ? <Button type="button" variant="outline" onClick={() => { setEditingKey(null); setForm({ ...blankForm(), unit: form.unit }); }}><X className="mr-2 h-4 w-4" />Hủy sửa</Button> : null}</div>
        </div>
      </Panel>
      <Panel title={config[category].label} description={`KH ${formatQuantity(totals.plan)} · TH ${formatQuantity(totals.actual)} ${config[category].metric}`}><div className="mb-3 grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-[1fr_1fr_auto_auto]"><Field label="Từ ngày"><Input type="date" value={fromDate} max={toDate || undefined} onChange={event => setFromDate(event.target.value)} /></Field><Field label="Đến ngày"><Input type="date" value={toDate} min={fromDate || undefined} onChange={event => setToDate(event.target.value)} /></Field><Button type="button" variant="outline" className="self-end" disabled={!fromDate && !toDate} onClick={() => { setFromDate(""); setToDate(""); }}>Đặt lại</Button><Button type="button" variant="outline" className="self-end" onClick={exportCurrentBoard}><Download className="mr-2 h-4 w-4" />Xuất Excel bảng này</Button></div>            <div className="overflow-x-auto"><table className="w-full min-w-[650px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="p-3">Ngày</th><th className="p-3">Đội / Vườn</th><th className="p-3 text-right">KH</th><th className="p-3 text-right">{isTapping ? "Cạo xong" : "TH"}</th>{isTapping ? <><th className="p-3 text-right">Chưa cạo</th><th className="p-3 text-right">Cạo chưa xong</th><th className="p-3 text-right">% hoàn thành</th><th className="p-3">Cạo tiếp vườn</th><th className="p-3 text-right">KH tiếp</th><th className="p-3 text-right">TH tiếp</th></> : <><th className="p-3 text-right">Lũy kế</th><th className="p-3 text-right">% hoàn thành</th></>}<th className="p-3">Thao tác</th></tr></thead><tbody>{orderedRecords.map(row => <tr key={row.id} className="border-b border-slate-100"><td className="p-3">{new Date(row.activityDate).toLocaleDateString("vi-VN")}</td><td className="p-3"><b>{row.unit}</b><span className="block text-xs text-slate-500">{row.gardenName}{(category === "care" || category === "treatment" || category === "fertilization") && row.workContent ? ` · ${row.workContent}` : ""}</span></td><td className="p-3 text-right">{formatQuantity(row.planQuantity)}</td><td className="p-3 text-right">{formatQuantity(row.actualQuantity)}</td>{isTapping ? <><td className="p-3 text-right">{formatQuantity(row.pendingGardens ?? 0)}</td><td className="p-3 text-right">{formatQuantity(row.partialGardens ?? 0)}</td><td className="p-3 text-right font-semibold text-emerald-700">{formatPercent(row.progressPercent)}</td><td className="p-3">{row.nextGarden ?? "—"}</td><td className="p-3 text-right">{formatQuantity(row.nextGardenPlanQuantity ?? 0)}</td><td className="p-3 text-right">{formatQuantity(row.nextGardenActualQuantity ?? 0)}</td></> : <><td className="p-3 text-right">{formatQuantity(row.cumulativeQuantity)}</td><td className="p-3 text-right font-semibold text-emerald-700">{formatPercent(row.progressPercent)}</td></>}<td className="p-3"><Button type="button" variant="outline" size="sm" onClick={() => loadRecord(row)}><Pencil className="mr-1 h-3 w-3" />Sửa</Button></td></tr>)}</tbody></table></div>{!records.length && !isLoading ? <p className="py-12 text-center text-sm text-slate-500">Chưa có dữ liệu cho bảng này.</p> : records.length && !orderedRecords.length ? <p className="py-12 text-center text-sm text-slate-500">Không có dữ liệu trong khoảng thời gian đã chọn.</p> : null}      </Panel>
      {isTapping ? <Panel title="Tổng hợp cạo mủ trong tháng" description="Tổng từ ngày đầu tháng đến hiện tại, phân theo Đội và Vườn; không cộng lặp bản ghi cùng ngày."><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead className="bg-emerald-50 text-left text-xs uppercase text-emerald-800"><tr><th className="p-3">Đội</th><th className="p-3">Vườn</th><th className="p-3 text-right">KH</th><th className="p-3 text-right">Cạo xong</th><th className="p-3 text-right">Chưa cạo</th><th className="p-3 text-right">Cạo chưa xong</th><th className="p-3 text-right">% hoàn thành</th><th className="p-3">Cạo tiếp vườn</th><th className="p-3 text-right">KH tiếp</th><th className="p-3 text-right">TH tiếp</th></tr></thead><tbody>{monthTappingRows.map(row => <tr key={`${row.unit}|${row.gardenName}`} className="border-b border-slate-100"><td className="p-3 font-semibold">{row.unit}</td><td className="p-3">{row.gardenName}</td><td className="p-3 text-right">{formatQuantity(row.plan)}</td><td className="p-3 text-right">{formatQuantity(row.actual)}</td><td className="p-3 text-right">{formatQuantity(row.pending)}</td><td className="p-3 text-right">{formatQuantity(row.partial)}</td><td className="p-3 text-right font-semibold text-emerald-700">{row.plan > 0 ? formatPercent((row.actual / row.plan) * 100) : "—"}</td><td className="p-3">{row.nextGarden || "—"}</td><td className="p-3 text-right">{formatQuantity(row.nextPlan)}</td><td className="p-3 text-right">{formatQuantity(row.nextActual)}</td></tr>)}</tbody></table></div>{!monthTappingRows.length ? <p className="py-8 text-center text-sm text-slate-500">Chưa có dữ liệu cạo mủ từ đầu tháng.</p> : null}</Panel> : null}
    </div>
  </div>;
}
