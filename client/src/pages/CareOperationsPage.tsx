...
export default function CareOperationsPage() {
  ...
  return <div className="page-enter">
    ...
    <div className="mt-4 grid gap-3 lg:grid-cols-[18rem_minmax(0,1fr)] xl:grid-cols-[20rem_minmax(0,1fr)]">
      ...
      <Panel compact className="w-full lg:col-span-2" title="Ghi chú theo ngày và nội dung" description="Hiển thị ghi chú đã nhập theo ngày, nội dung công việc và thứ tự Đội; ghi chú không nằm trong cột Thao tác."><div className="grid gap-2">{noteRows.length ? noteRows.map(row => <div key={`note-${row.id}`} className="grid gap-2 rounded-lg border border-slate-200 bg-white p-2 sm:grid-cols-[7rem_7rem_minmax(10rem,1fr)_minmax(12rem,1fr)_auto] sm:items-center"><div className="text-xs text-slate-600">{new Date(row.activityDate).toLocaleDateString("vi-VN")}</div><div><b>{row.unit}</b><span className="block text-xs text-slate-500">{row.gardenName}</span></div><div className="text-sm font-medium text-slate-700">{row.workContent || config[row.category].label}</div><Input defaultValue={row.note ?? ""} placeholder="Thêm ghi chú cho nội dung này" aria-label={`Ghi chú ${row.unit} ${row.workContent ?? config[row.category].label}`} /><Button type="button" variant="outline" className="self-end" onClick={event => { const input = event.currentTarget.parentElement?.querySelector("input"); saveRecordNote(row, input?.value ?? ""); }} disabled={save.isPending}><Save className="mr-1 h-3 w-3" />{row.note ? "Sửa ghi chú" : "Thêm ghi chú"}</Button></div>) : <p className="py-4 text-center text-sm text-slate-500">Chưa có bản ghi trong phạm vi ngày đang chọn để hiển thị ghi chú.</p>}</div></Panel>
    </div>
  </div>;
}
