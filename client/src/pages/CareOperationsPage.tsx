...
export default function CareOperationsPage() {
  ...
  const { data: teamNotes = [] } = trpc.operations.teamNotes.useQuery();
  ...
  const saveTeamNote = trpc.operations.saveTeamNote.useMutation({
    onSuccess: async () => { await utils.operations.teamNotes.invalidate(); toast.success("Đã lưu ghi chú theo Đội"); },
    onError: error => toast.error(error.message),
  });
  ...
  const noteDate = fromDate || latestCareDate(records) || form.date;
  const noteRows = useMemo(() => {
    const inRange = teamNotes.filter(note => filterCareRecordsByDateRange([{ activityDate: note.activityDate } as never], fromDate || undefined, toDate || undefined).length > 0);
    const byKey = new Map(inRange.map(note => [`${dateKey(note.activityDate)}|${note.unit}`, { activityDate: dateKey(note.activityDate), unit: note.unit, note: note.note }]));
    const visibleTeams = Array.from(new Set(filteredRecords.map(row => row.unit))).sort(compareTeamName);
    visibleTeams.forEach(unit => { const key = `${noteDate}|${unit}`; if (!byKey.has(key)) byKey.set(key, { activityDate: noteDate, unit, note: "" }); });
    return Array.from(byKey.values()).sort((left, right) => right.activityDate.localeCompare(left.activityDate) || compareTeamName(left.unit, right.unit));
  }, [teamNotes, fromDate, toDate, filteredRecords, noteDate]);
  ...
  return <div className="page-enter">
    ...
    <div className="mt-4 grid gap-3 lg:grid-cols-[18rem_minmax(0,1fr)] xl:grid-cols-[20rem_minmax(0,1fr)]">
      ...
      <Panel compact className="w-full lg:col-span-2" title="Ghi chú theo Đội" description={`Ghi chú của ngày ${noteDate || "đang chọn"}; sắp xếp theo Đội và chỉ người có quyền care:write mới lưu được.`}><div className="grid gap-2">{noteRows.length ? noteRows.map(row => { const key = `${row.activityDate}|${row.unit}`; const value = noteDrafts[key] ?? row.note; return <div key={key} className="grid gap-2 rounded-lg border border-slate-200 bg-white p-2 sm:grid-cols-[8rem_minmax(0,1fr)_auto] sm:items-center"><div><b>{row.unit}</b><span className="block text-xs text-slate-500">{new Date(`${row.activityDate}T00:00:00`).toLocaleDateString("vi-VN")}</span></div><Input value={value} onChange={event => setNoteDrafts(current => ({ ...current, [key]: event.target.value }))} placeholder="Bổ sung ghi chú, khó khăn hoặc kiến nghị" /><Button type="button" variant="outline" className="self-end" disabled={saveTeamNote.isPending} onClick={() => saveTeamNote.mutate({ activityDate: new Date(`${row.activityDate}T00:00:00.000Z`), unit: row.unit, note: value })}><Save className="mr-1 h-3 w-3" />Lưu ghi chú</Button></div>; }) : <p className="py-4 text-center text-sm text-slate-500">Chưa có Đội trong phạm vi ngày đang chọn để ghi chú.</p>}</div></Panel>
    </div>
  </div>;
}
