...
const GUIDE_PDF_URL =
  "/manus-storage/huong-dan-su-dung-cao-su-cn386_b89a88de.pdf";
const EXCEL_BUNDLE_URL =
  "/manus-storage/bo-mau-excel-import-export-cao-su-cn386_1eb441e7.zip";
...
const chapters: GuideChapter[] = [
  {
    ...
    summary:
      "Cài Cao su CN386 như một ứng dụng độc lập, có biểu tượng trên màn hình chính.",
    keywords:
      "cài đặt máy tính laptop chrome edge android ios iphone ipad pwa màn hình chính",
    icon: Smartphone,
    steps: [
    ...
    notes: [
      "Nếu đang mở link trong Zalo/Facebook, hãy chọn Mở bằng Chrome hoặc Safari trước khi cài.",
    ],
    image: images.install,
    imageAlt: "Trang Cài ứng dụng Cao su CN386",
    caption:
      "Hệ thống → Cài ứng dụng có nút cài, hướng dẫn thủ công và sao chép liên kết.",
  },
  {
    id: "login",
    title: "Đăng nhập, đăng xuất và phạm vi quyền",
    summary:
      "Dùng tài khoản cá nhân do quản trị viên cấp; dữ liệu và nút thao tác thay đổi theo quyền.",
    ...
    notes: [
      "Không dùng chung mật khẩu; không gửi mật khẩu trong ảnh chụp lỗi.",
    ],
  },
  {
    id: "lookup",
    ...
    summary:
      "Luôn kiểm tra kỳ xem trước khi đọc số liệu: Năm, Tháng/Cả năm, Đợt, Đội hoặc khoảng ngày.",
    keywords:
      "tổng quan vườn lọc tra cứu năm tháng cả năm đợt đội từ ngày đến ngày",
    icon: Filter,
    steps: [
    ...
    caption:
      "Menu gồm Điều hành, Báo cáo và Hệ thống; nội dung hiển thị theo quyền tài khoản.",
  },
  {
    id: "latex",
    title: "Ghi nhận, sửa, xóa và xuất Nhật ký nhập – xuất mủ",
    summary:
      "Nhập theo Đội/Vườn, theo dõi nhật ký, tra cứu theo kỳ và sửa hoặc xóa đúng bản ghi.",
    keywords:
      "nhập mủ xuất mủ sửa xóa nhật ký đợt vườn lô mủ đông mủ dây hao kho",
    icon: Sprout,
    steps: [
    ...
    notes: [
      "Sau khi sửa hoặc xóa, lọc lại cùng kỳ và đối chiếu dòng Tổng cộng.",
    ],
  ...
  },
  {
    ...
    summary:
      "Theo dõi cạo mủ, rập thiết kế, chăm sóc, phun/bôi thuốc và bón phân theo ngày và Đội.",
    keywords:
      "khai thác chăm sóc cạo mủ rập thiết kế phun bôi thuốc bón phân ghi chú ngày",
    icon: Sprout,
    steps: [
    ...
    notes: [
      "Ngày lọc và ngày hiển thị dùng ngày nghiệp vụ theo múi giờ Việt Nam.",
    ],
    image: images.care,
    imageAlt: "Trang Khai thác và chăm sóc hằng ngày",
    caption:
      "Form nhập, bộ lọc ngày, bảng theo dõi và các nút Excel trên cùng màn hình.",
  },
  {
    id: "workforce",
    title: "Quản lý nhân công và đánh giá tay nghề",
    summary:
      "Theo dõi biên chế, trạng thái, mã số, phân công, đánh giá kỹ thuật và hao dăm.",
    keywords:
      "nhân công biên chế mã số phân công tay nghề hao dăm import đánh giá",
    icon: UsersRound,
    steps: [
    ...
    caption:
      "Các nút xuất/import cùng khối tổng hợp biên chế và trạng thái nhân công.",
  },
  {
    id: "reports",
    title: "Báo cáo và xuất số liệu",
    summary:
      "Đọc đúng kỳ và xuất báo cáo tiến độ, hao hụt kho, tăng giảm, sản lượng lô và tay nghề.",
    keywords:
      "báo cáo tiến độ hao hụt kho tăng giảm sản lượng lô tay nghề csv excel",
    icon: BarChart3,
    steps: [
  ...
  },
  {
    ...
    summary:
      "Tải đúng mẫu, giữ cấu trúc, xem trước lỗi theo dòng rồi mới xác nhận nhập.",
    keywords:
      "import export excel mẫu zip chọn tệp tải mẫu dòng lỗi sheet xem trước",
    ...
    notes: [
      "Gói ZIP gồm 14 mẫu import và 9 cấu trúc export; mẫu có danh mục Lô động vẫn nên tải mới trực tiếp từ phần mềm.",
    ],
    image: images.excel,
  ...
  },
  {
    ...
    summary:
      "Quy trình xử lý lỗi theo dòng, sửa/xóa có kiểm tra, export đối chiếu và sao lưu dữ liệu.",
    keywords:
      "sửa lỗi lưu trữ sao lưu backup dòng lỗi không nhận file không thấy nút ngày lệch",
    icon: Wrench,
    steps: [
    ...
    notes: [
      "Khi báo lỗi cho quản trị viên, gửi tên tài khoản, chức năng, Đội, tên file, thời điểm và ảnh thông báo; không gửi mật khẩu.",
    ],
  },
];
  ...
  return (
    <div className={`rounded-2xl border p-4 ${styles}`}>
      ...
      <a
        href={href}
        download={fileName}
        className="mt-4 inline-flex h-11 w-full items-center justify-center rounded-xl bg-white px-4 text-sm font-semibold shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
      >
...
function GuideChapterCard({ chapter }: { chapter: GuideChapter }) {
  ...
  return (
    <article
      id={chapter.id}
      className="scroll-mt-24 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_12px_30px_-24px_rgba(15,23,42,0.45)]"
    >
      <div className="p-4 sm:p-6">
        ...
        <ol className="mt-5 grid gap-3">
          {chapter.steps.map((step, index) => (
            <li
              key={step}
              className="flex min-w-0 gap-3 text-sm leading-6 text-slate-700"
            >
              <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-emerald-700 text-xs font-bold text-white">
                {index + 1}
              </span>
        ...
        {chapter.notes?.map(note => (
          <div
            key={note}
            className="mt-4 flex min-w-0 gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm leading-5 text-amber-900"
          >
      ...
      {chapter.image ? (
        <figure className="border-t border-slate-200 bg-slate-50 p-3 sm:p-5">
          <img
            src={chapter.image}
          ...
          />
          <figcaption className="mt-2 break-words text-center text-xs leading-5 text-slate-500">
            {chapter.caption}
          </figcaption>
        </figure>
      ) : null}
  ...
  return (
    <div className="page-enter">
      <PageHeader
        ...
        description="Tra cứu cách cài đặt, đăng nhập, theo dõi, nhập liệu, import/export Excel, sửa lỗi và sao lưu dữ liệu Cao su CN386."
        action={
          <a
            href={GUIDE_PDF_URL}
            download="huong-dan-su-dung-cao-su-cn386.pdf"
            className="block w-full sm:w-auto"
          >
            <Button className="h-11 w-full bg-emerald-700 hover:bg-emerald-800 sm:w-auto">
              <Download className="mr-2 h-4 w-4" />
              Tải hướng dẫn PDF
            </Button>
          </a>
        ...
        <div className="grid gap-5 p-5 sm:p-7 lg:grid-cols-[1fr_360px] lg:items-center">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-emerald-300">
              ...
              Cẩm nang Cao su CN386
            </div>
            <h2 className="mt-3 break-words font-display text-2xl font-bold tracking-tight sm:text-3xl">
              Tìm nhanh thao tác, tải tài liệu và bộ mẫu
            </h2>
            <p className="mt-3 max-w-2xl break-words text-sm leading-6 text-emerald-50/75">
              Tài liệu PDF 14 trang có ảnh minh họa. Gói ZIP gồm 14 mẫu import
              và 9 cấu trúc export, không chứa dữ liệu sản xuất thật.
            </p>
            <div className="mt-5 grid gap-2 sm:flex sm:flex-wrap">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-emerald-400 sm:w-auto px-4 text-sm font-bold text-emerald-950 transition hover:bg-emerald-300"
              >
                <BookOpenCheck className="mr-2 h-4 w-4" />
                Mở PDF
              </a>
              <a
                href={EXCEL_BUNDLE_URL}
                download="bo-mau-excel-import-export-cao-su-cn386.zip"
                className="inline-flex min-h-11 w-full items-center justify-center rounded-xl border border-white/20 bg-white/10 px-4 text-sm font-semibold text-white transition hover:bg-white/15"
              >
              ...
              <a
                href="/install"
                className="inline-flex h-10 items-center rounded-xl border border-white/20 bg-white/10 px-4 text-sm font-semibold text-white transition hover:bg-white/15"
              >
          ...
          <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur">
            ...
            <div className="relative mt-2">
              ...
              <Input
                ...
                placeholder="VD: import, ngày, cài iPhone…"
                className="h-11 border-white/20 bg-white pl-9 text-slate-950"
              />
            </div>
            <p className="mt-2 text-xs text-emerald-50/65">
              {visibleChapters.length} mục phù hợp
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="mb-5 grid gap-3 md:grid-cols-2">
        <DownloadCard
          ...
          description="14 trang, có ảnh minh họa, phù hợp để in hoặc gửi nội bộ."
          href={GUIDE_PDF_URL}
          fileName="huong-dan-su-dung-cao-su-cn386.pdf"
          tone="emerald"
        />
        <DownloadCard
          icon={Archive}
          title="Bộ ZIP mẫu Import & Export"
          description="23 workbook mẫu và README; không chứa dữ liệu sản xuất thật."
          href={EXCEL_BUNDLE_URL}
          fileName="bo-mau-excel-import-export-cao-su-cn386.zip"
          tone="sky"
        />
      </div>
      ...
      <Panel
        compact
        className="mb-5"
        title="Đi đến nội dung"
        description="Chọn mục để cuộn nhanh; hoặc dùng ô tìm kiếm phía trên."
      >
        <nav aria-label="Mục lục hướng dẫn" className="flex flex-wrap gap-2">
          {chapters.map(chapter => (
            <a
              key={chapter.id}
              href={`#${chapter.id}`}
              className="inline-flex min-h-11 w-full items-center justify-center rounded-full border border-slate-200 bg-white px-3 py-2 text-center text-xs font-semibold leading-4 sm:w-auto text-slate-700 transition hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-800"
            >
              {chapter.title}
            </a>
          ))}
      ...
      {visibleChapters.length ? (
        <div className="grid gap-5">
          {visibleChapters.map(chapter => (
            <GuideChapterCard key={chapter.id} chapter={chapter} />
          ))}
        </div>
      ) : (
        <Panel>
          <div className="py-10 text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Thử từ khóa khác như “import”, “đăng nhập”, “ngày”, “báo cáo” hoặc
              “sao lưu”.
            </p>
            <Button
              variant="outline"
      ...
      <section className="mt-5 grid gap-3 md:grid-cols-3">
        <a
          href="/install"
          className="min-h-[44px] min-w-0 rounded-2xl border border-slate-200 bg-white p-4 transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md"
        >
          <Laptop className="h-5 w-5 text-emerald-700" />
          <p className="mt-3 break-words font-semibold text-slate-900">
            Cài ứng dụng
          </p>
          <p className="mt-1 break-words text-sm leading-5 text-slate-500">
            Mở trang cài PWA cho máy tính và điện thoại.
          </p>
        </a>
        <a
          href="/data-tools"
        ...
        >
          ...
          <p className="mt-3 font-semibold text-slate-900">Import & Excel</p>
          <p className="mt-1 text-sm leading-5 text-slate-500">
            Tải mẫu mới nhất, import, xuất toàn bộ và sao lưu.
          </p>
        </a>
        <a
        ...
        >
          ...
          <p className="mt-1 text-sm leading-5 text-slate-500">
            Mở hướng dẫn chính thức của Google Chrome.
          </p>
        </a>
      </section>
    </div>
