...
export default function DataToolsPage() {
  ...
  const [restorePreview, setRestorePreview] = useState<{ sheets: string[]; rows: number } | null>(null);
  ...
  const backupRestore = trpc.dataTools.backups.restore.useMutation({
    onSuccess: async result => { await utils.dataTools.summary.invalidate(); setRestoreFile(null); setRestorePreview(null); toast.success(`Đã khôi phục ${result.total.toLocaleString("vi-VN")} bản ghi. Dữ liệu trùng khóa đã được cập nhật.`); },
    onError: error => toast.error(error.message),
  });
  const inspectRestoreFile = async (event: ChangeEvent<HTMLInputElement>) => {
    const selected = event.target.files?.[0];
    if (!selected) return;
    try {
      const XLSX = await import("xlsx");
      const book = XLSX.read(await selected.arrayBuffer(), { type: "array", cellDates: false });
      const rows = book.SheetNames.reduce((sum, name) => sum + XLSX.utils.sheet_to_json(book.Sheets[name], { header: 1, defval: "" }).length - 1, 0);
      setRestoreFile(selected); setRestorePreview({ sheets: book.SheetNames, rows: Math.max(0, rows) });
    } catch { setRestoreFile(null); setRestorePreview(null); toast.error("Không đọc được file. Hãy chọn file .xlsx được tạo từ Sao lưu dữ liệu."); }
  };
  const restoreBackup = async () => {
    if (!restoreFile) return;
    const accepted = window.confirm("CẢNH BÁO KHÔI PHỤC\n\nHệ thống sẽ cập nhật các bản ghi trùng khóa từ file backup và không xóa dữ liệu hiện có. Một số sheet bảo mật (tài khoản, nhật ký) sẽ không được khôi phục. Bạn có chắc chắn muốn tiếp tục?");
    if (!accepted) return;
    ...
    backupRestore.mutate({ contentBase64: btoa(binary) });
  };
