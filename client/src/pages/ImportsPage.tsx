    ...
    <div className="grid gap-5 xl:grid-cols-[0.92fr_1.4fr]">
      <Panel title={editingId ? "Sửa ghi nhận nhập mủ" : "Ghi nhận nhập mủ"} description="Chọn Đội và Vườn A/B/C hoặc tất cả. Lô chỉ cần chọn khi muốn theo dõi chi tiết; bỏ trống Lô vẫn ghi nhận được theo Vườn/Đội.">
        {!plots?.length ? <EmptyState title="Cần khai báo vườn trước" description="Hãy yêu cầu quản trị viên thêm vườn rồi quay lại ghi nhận nhập mủ." /> : <form onSubmit={submit} className="grid gap-4">
          <div className="grid gap-4 sm:grid-cols-3"><Field label="Đội"><select value={unit} onChange={e => { setUnit(e.target.value); setPlotId(""); }} required className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"><option value="">Chọn đội</option>{units.map(item => <option key={item} value={item}>{item}</option>)}</select></Field><Field label="Vườn"><select value={gardenType} onChange={e => { setGardenType(e.target.value); setPlotId(""); }} required disabled={!unit} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"><option value="all">Tất cả vườn</option><option value="A">Vườn A</option><option value="B">Vườn B</option><option value="C">Vườn C</option></select></Field><Field label="Lô (không bắt buộc)"><select value={plotId} onChange={e => setPlotId(e.target.value)} disabled={!unit} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"><option value="">Không chọn Lô · lưu theo {gardenLabel}</option>{selectablePlots.map(plot => <option key={plot.id} value={plot.id}>{plot.name}</option>)}</select></Field></div>
          <div className="grid gap-4 sm:grid-cols-2"><Field label="Ngày nhập"><Input type="date" value={recordDate} onChange={e => setRecordDate(e.target.value)} required /></Field><Field label="Đợt"><select value={periodLabel} onChange={e => setPeriodLabel(e.target.value)} required className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring">{STANDARD_PERIODS.map(period => <option key={period} value={period}>{period}</option>)}</select></Field></div>
          <div className="grid gap-4 sm:grid-cols-2"><Field label="Mủ đông (kg)"><Input type="number" min="0" step="0.01" value={frozenLatex} onChange={e => setFrozenLatex(e.target.value)} placeholder="0.00" required /></Field><Field label="Mủ dây (kg)"><Input type="number" min="0" step="0.01" value={latexThread} onChange={e => setLatexThread(e.target.value)} placeholder="0.00" required /></Field></div>
          ...
          <Field label="Ghi chú"><Textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Ghi chú cho bản ghi (nếu có)" /></Field>
          <div className="flex flex-wrap gap-2"><Button disabled={isMutating} className="mt-1 bg-emerald-700 hover:bg-emerald-800">{isMutating ? "Đang xử lý…" : editingId ? <><Save className="mr-2 h-4 w-4" />Cập nhật nhập mủ</> : <><Save className="mr-2 h-4 w-4" />Lưu nhập mủ</>}</Button>{editingId ? <><Button type="button" variant="outline" className="border-red-200 text-red-700 hover:bg-red-50" onClick={confirmRemove} disabled={isMutating}><Trash2 className="mr-2 h-4 w-4" />Xóa bản ghi</Button><Button type="button" variant="outline" onClick={cancelEditing}><X className="mr-2 h-4 w-4" />Hủy sửa</Button></> : null}</div>
        </form>}
      </Panel>
      <Panel title="Nhật ký nhập mủ" description="Tra cứu và xuất báo cáo theo Đợt, khoảng thời gian; có thể sửa hoặc xóa từng bản ghi theo quyền được cấp.">
        <div className="mb-4 grid gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3 sm:grid-cols-3 xl:grid-cols-5"><Field label="Đợt"><select value={journalPeriod} onChange={event => setJournalPeriod(event.target.value)} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"><option value="">Tất cả Đợt</option>{journalPeriods.map(period => <option key={period} value={period}>{period}</option>)}</select></Field><Field label="Từ ngày"><Input type="date" value={fromDate} max={toDate || undefined} onChange={event => setFromDate(event.target.value)} /></Field><Field label="Đến ngày"><Input type="date" value={toDate} min={fromDate || undefined} onChange={event => setToDate(event.target.value)} /></Field><div className="flex items-end"><Button type="button" variant="outline" className="w-full" onClick={() => { setJournalPeriod(""); setFromDate(""); setToDate(""); }}><RotateCcw className="mr-2 h-4 w-4" />Đặt lại</Button></div><div className="flex items-end"><Button type="button" className="w-full bg-emerald-700 hover:bg-emerald-800" onClick={exportJournal}><Download className="mr-2 h-4 w-4" />Xuất Excel</Button></div></div>
        {isLoading ? <div className="h-56 animate-pulse rounded-xl bg-slate-100" /> : !filteredRecords.length ? <EmptyState title="Chưa có bản ghi phù hợp" description={journalPeriod || fromDate || toDate ? "Hãy thay đổi Đợt hoặc khoảng ngày để tra cứu." : "Dữ liệu nhập mủ hằng ngày sẽ xuất hiện tại đây."} /> : <div className="overflow-x-auto"><table className="w-full min-w-[860px] text-left"><thead><tr className="border-b border-slate-200 text-xs font-bold uppercase tracking-[0.1em] text-slate-400"><th className="px-3 py-3">Ngày</th><th className="px-3 py-3">Vườn / đơn vị</th><th className="px-3 py-3">Đợt</th><th className="px-3 py-3 text-right">Mủ đông</th><th className="px-3 py-3 text-right">Mủ dây</th><th className="px-3 py-3 text-right">Cộng nhập</th><th className="px-3 py-3">Thao tác</th></tr></thead><tbody>{filteredRecords.map(row => <tr key={`${row.source}-${row.id}`} className="border-b border-slate-100 last:border-0"><td className="px-3 py-4 text-sm text-slate-600">{formatDate(row.recordDate)}</td><td className="px-3 py-4"><p className="font-semibold text-slate-800">{row.plotId ? `${row.plotCode} — ` : ""}{row.plotName}</p><p className="mt-0.5 text-xs text-slate-400">{row.unit}</p></td><td className="px-3 py-4 text-sm text-slate-600">{row.periodLabel}</td><td className="px-3 py-4 text-right text-sm">{formatQuantity(row.frozenLatex)}</td><td className="px-3 py-4 text-right text-sm">{formatQuantity(row.latexThread)}</td><td className="px-3 py-4 text-right font-bold text-emerald-700">{formatQuantity(row.totalImport)} kg</td><td className="px-3 py-4"><Button type="button" variant="outline" size="sm" onClick={() => loadRecord(row)}><Pencil className="mr-1 h-3 w-3" />Sửa</Button></td></tr>)}</tbody></table></div>}
      </Panel>
    </div>
  </div>;
}
