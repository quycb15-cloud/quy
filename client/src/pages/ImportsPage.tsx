...
export default function ImportsPage() {
  ...
  const orderedRecords = useMemo(() => [...(records ?? [])].sort((left, right) => new Date(right.recordDate).getTime() - new Date(left.recordDate).getTime() || comparePeriodLabel(left.periodLabel, right.periodLabel) || compareTeamName(left.unit, right.unit)), [records]);
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); save.mutate({ plotId: plotId ? Number(plotId) : null, unit, gardenName: gardenLabel, periodLabel, recordDate: new Date(`${recordDate}T12:00:00`), frozenLatex: Number(frozenLatex), latexThread: Number(latexThread), note }); };
