...
export default function ProductionChangePage() {
  ...
  return <div className="page-enter">
    <PageHeader eyebrow="Phân tích sản lượng" title="Báo cáo tăng, giảm" description="Tổng hợp theo Đội, kỳ và tháng; chỉ hiển thị Hao kho được tính bằng Nhập − Xuất." action={<Button variant="outline" className="bg-white" onClick={exportExcel} disabled={!rows.length}><Download className="mr-2 h-4 w-4" />Xuất Excel</Button>} />
...
function MobileCard({ row }: { row: { periodLabel: string; monthLabel: string; unit: string; totalImport: number; totalExport: number } }) { const warehouseLoss = row.totalImport - row.totalExport; return <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm"><div><p className="font-semibold text-slate-900">{row.unit}</p><p className="text-xs text-slate-500">{row.periodLabel} · Tháng {row.monthLabel}</p></div><div className="mt-3 grid grid-cols-3 gap-3 text-sm"><Metric label="Cộng nhập" value={`${formatQuantity(row.totalImport)} kg`} /><Metric label="Cộng xuất" value={`${formatQuantity(row.totalExport)} kg`} /><Metric label="Hao kho" value={`${formatQuantity(warehouseLoss)} kg`} tone={warehouseLoss >= 0 ? "text-amber-700" : "text-rose-700"} /></div></div>; }
