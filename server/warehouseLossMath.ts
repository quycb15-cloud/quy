...
export function aggregateWarehouseLoss(imports: WarehouseImport[], exports: WarehouseExport[], periodLabel?: string, monthLabel?: string) {
  ...
  const matches = (row: WarehouseImport | WarehouseExport) => (!periodLabel || periodLabel === "All" || row.periodLabel === periodLabel) && (!monthLabel || monthLabelOf(row.recordDate) === monthLabel);
  ...
  const monthFilteredRecords = [...imports, ...exports].filter(row => (!monthLabel || monthLabelOf(row.recordDate) === monthLabel) && (!periodLabel || periodLabel === "All" || row.periodLabel === periodLabel));
